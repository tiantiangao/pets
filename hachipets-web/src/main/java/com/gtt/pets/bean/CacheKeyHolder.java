package com.gtt.pets.bean;

/**
 * 缓存key定义
 *
 * @author tiantiangao
 */
public class CacheKeyHolder {

    /**
     * 全局配置
     */
    public static final String GLOBAL_CONFIG = "oGlobalConfig";
    /**
     * 宠物电影
     */
    public static final String MOVIE = "oMovie";
    /**
     * 宠物电影的外部网站介绍信息
     */
    public static final String MOVIE_INFO_LIST = "oMovieInfoList";
    /**
     * 宠物电影的播放信息
     */
    public static final String MOVIE_PLAY_LIST = "oMoviePlayList";
    /**
     * 热门宠物电影
     */
    public static final String MOVIE_HOT_LIST = "oMovieHotList";
    /**
     * 最新宠物电影
     */
    public static final String MOVIE_NEW_LIST = "oMovieNewList";
    /**
     * 推荐宠物电影
     */
    public static final String MOVIE_RECOMMEND = "oMovieRecommend";
    /**
     * 宠物电影地区列表
     */
    public static final String MOVIE_REGION_LIST = "oMovieRegionList";
    /**
     * 宠物电影地区
     */
    public static final String MOVIE_REGION = "oMovieRegion";
    /**
     * 宠物电影年代列表
     */
    public static final String MOVIE_YEAR_LIST = "oMovieYearList";
    /**
     * 宠物电影年代
     */
    public static final String MOVIE_YEAR = "oMovieYear";
    /**
     * 宠物分类
     */
    public static final String CATEGORY = "oCategory";
    /**
     * 宠物分类列表
     */
    public static final String CATEGORY_LIST = "oCategoryList";
    /**
     * 宠物FCI分组
     */
    public static final String BAIKE_FCI_GROUP = "oBaikeFCIGroup";
    /**
     * 宠物FCI分类
     */
    public static final String BAIKE_FCI_SECTION = "oBaikeFCISection";
    /**
     * 宠物FCI分组列表
     */
    public static final String BAIKE_FCI_GROUP_LIST  = "oBaikeFCIGroupList";
    /**
     * 宠物FCI分类列表
     */
    public static final String BAIKE_FCI_SECTION_LIST = "oBaikeFCISectionList";
}
